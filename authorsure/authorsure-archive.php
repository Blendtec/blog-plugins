<?php
add_action('init', array(AUTHORSURE_ARCHIVE,'admin_init'));

class authorsure_archive {
    
	static function admin_init() {
		add_action('load-edit-tags.php', array(AUTHORSURE_ARCHIVE, 'load_page'));	
		add_action( 'edit_term', array(AUTHORSURE_ARCHIVE, 'save'), 10, 2 );	
	}

	static function load_page() {
		add_action( $_REQUEST['taxonomy'] . '_edit_form', array(AUTHORSURE_ARCHIVE, 'archive_panel'), 10, 2 );
...
<p>Notes on getting your face to appear on archive pages</p><ul><li>Make sure you WordPress theme is not "noindexing" your category/tag pages</li>
<li>Make sure you include a paragraph of keyword rich and compelling copy as the intro to the archive section: often your theme will allow 
