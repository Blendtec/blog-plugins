...
class authorsure_admin {
	...
	static function load_page() {
		...
		add_meta_box('authorsure-rel-author', __('Post Settings (rel="author")',AUTHORSURE), array(AUTHORSURE_ADMIN, 'post_panel'), self::get_screen_id(), 'normal', 'core');
		add_meta_box('authorsure-archive', __('Archive Settings (rel="author")',AUTHORSURE), array(AUTHORSURE_ADMIN, 'archive_panel'), self::get_screen_id(), 'normal', 'core');
		add_meta_box('authorsure-rel-home', __('Home Page Settings (rel="author" and rel="publisher")',AUTHORSURE), array(AUTHORSURE_ADMIN, 'home_panel'), self::get_screen_id(), 'normal', 'core');
		add_meta_box('authorsure-rel-me', __('Author Page Settings (rel="me")',AUTHORSURE), array(AUTHORSURE_ADMIN, 'author_panel'), self::get_screen_id(), 'normal', 'core');
		add_meta_box('authorsure-advanced', __('Author Page Advanced Settings (rel="me")',AUTHORSURE), array(AUTHORSURE_ADMIN, 'advanced_panel'), self::get_screen_id(), 'normal', 'core');
		add_meta_box('authorsure-role', __('Minimum Roles',AUTHORSURE), array(AUTHORSURE_ADMIN, 'role_panel'), self::get_screen_id(), 'normal', 'core');
...
TOGGLE_POSTBOXES;
	...
	static function save() {
    	...
    	foreach ($options as $option => $old_value) {
         	...
         	switch ($option) {
				case 'publisher_rel' : $options[$option] = authorsure::sanitize_publisher($options[$option]); break;
         	}
    	} //end for
...
<p>Select the author of the home page, first making sure that author has set up a Google+ link in their profile. This will put a rel="author" link to the specified Google+ profile on your home page, {$homepage} with the end result being that their face will appear in the SERPS in the entry for your home page.</p>
